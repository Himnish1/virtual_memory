...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    uintptr_t new_physical = find_nxt_free_pg(p->p_pid);
    // allocate memory
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        if (assign_physical_page(new_physical, p->p_pid) < 0
            || virtual_memory_map(p->p_pagetable, addr, new_physical, PAGESIZE,
    ...
    if ((ph->p_flags & ELF_PFLAG_WRITE) == 0) {
        vamapping start_of_segment = virtual_memory_lookup(p->p_pagetable, va);
        int check = virtual_memory_map(p->p_pagetable, va, PTE_ADDR(start_of_segment.pa), PAGESIZE, PTE_P | PTE_U, NULL);
    }
